#!/usr/bin/env bash
# Deploy the InstructLab UI quadlet (enable --now instead of start only)
set -euo pipefail
CONTAINER_FILE=${1:-instructlab-ui.container}
...
echo "Deployed ${SERVICE_NAME}. Visit http://127.0.0.1:8777  (token: instructlab)"
